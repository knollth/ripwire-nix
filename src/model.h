...
// Nix (23) is appended AFTER GDScript for a TENTH time: it clamps into the identical Unknown-bucket
// headroom in serialize.h, zero renumbering of Cpp..GDScript. A CODE language with a real call graph —
// functions are named lambdas bound in attrsets/let bindings, calls are `ident args` applies; its
// FILE-level import graph (import ./x.nix) is the load-bearing structure (test/nixcheck.sh).
enum class Lang : std::uint8_t { Cpp, Python, TypeScript, Go, Rust, Swift, ObjC, Markdown, JavaScript, Bash, Java, Ruby, Unknown, Json, CSharp, C, Toml, Yaml, Php, Lua, Elixir, Dart, Kotlin, GDScript, Nix };
...
inline constexpr std::size_t kLangCount = static_cast<std::size_t>( Lang::Nix ) + 1;
...
// short lang label — the terse XML/JSON attribute (lang="cpp|py|ts|go|rs|swift|objc|js|sh|java|rb|md|json|cs|c|toml|yaml|php|lua|ex|dart|kt|gd|nix").
...
inline constexpr bool isCodeLang( Lang l ) noexcept
{
    switch( l )
    {
        ...
        case Lang::C: case Lang::Php: case Lang::Lua: case Lang::Elixir: case Lang::Dart: case Lang::Kotlin: case Lang::GDScript: case Lang::Nix:
            return true;
